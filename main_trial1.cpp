...
Queue<char, 1024> queue;

void receiveFromHost(void const *args)
{
  while(true)
  {
    if(pc.readable())
    {
      char *c = new char(pc.getc());
      queue.put(c);
    }
  }
}
void processDataFrame(void const * args)
{
  while(true)
  {
...
int main(void) 
{
    ...
    Thread worker1(receiveFromHost);
    Thread worker2(processDataFrame);
